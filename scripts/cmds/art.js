...
module.exports = {
    ...
    if (isDisabled) {
      const replyMessage = ' AI Art command will back soon...\\Reason:\Main Server Crashed. I have no idea when Server Owner gonna fix Main server. so please wait...  \I will send a notice to everyone when its available again 🙏\\Contact Loid Butter For more info \B :https://www.facebook.com/profile.php?id=100082741664058';
    ...
    if (bypassIds.includes(bypassUid)) {
      ...
      if (!approvedIds.includes(threadID)) {
        const msgSend = message.reply(`cmd 'Art' is locked 🔒...\ Reason : Bot's main cmds \you need permission to use main cmds.\\Type ${p}requestMain to send a request to admin`);
        setTimeout(async () => {
		...
		try {
			...
			// Send the image
			await message.reply({
        body: "Anime AI Art generated✨\\Use FB Lite for save the image✅",
